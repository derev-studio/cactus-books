#!/usr/bin/env python3
"""
Перевод морфологии на украинский (morphology_* → morphology_*_uk).
Тільки вбудований Python — нічого встановлювати не потрібно.

Два безкоштовні API: Free Translate API + MyMemory (en → uk).
Той самий підхід, що й для російської. Запуск:

  cd /Users/alexanderermolovich/Documents/cactus
  python3 -u scripts/translate_morphology_to_uk.py
"""
...
def _translate_mymemory(text: str) -> str | None:
    ...
    try:
        url = MYMEMORY_URL + "?" + urllib.parse.urlencode({"q": text, "langpair": "en|uk"})
        req = urllib.request.Request(url, headers={"User-Agent": USER_AGENT})
        with urllib.request.urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
            data = json.loads(resp.read().decode())
...
def translate_en_uk(text: str) -> str | None:
...
def main(limit: int | None = None, only_genera: list[str] | None = None):
    ...
    print("Скрипт перевода на украинский запущен. Без установки библиотек.", flush=True)
    print("API: Free Translate + MyMemory (en → uk).", flush=True)
    ...
    for genus_file in sorted(SPECIES_DIR.glob("*.json")):
        ...
        for sp in species_list:
            ...
            for field in MORPH_FIELDS:
                en_val = sp.get(field)
                uk_field = field + "_uk"
                if not en_val or not (en_val := str(en_val).strip()) or sp.get(uk_field):
                    continue
    ...
    print()
    print("Готово. Переведено полей (укр.):", total_done, ", обновлено родов:", genera_saved, flush=True)
